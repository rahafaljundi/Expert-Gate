function [net, info] = cnn_train_adagrad_oneLayer(net,input_net, imdb, getBatch, varargin)
% cnn_train_adagrad_oneLayer   Demonstrates training a CNN using adagrad
opts.train = [] ;
opts.val = [] ;
opts.numEpochs = 300 ;
opts.batchSize = 256 ;
...
opts.dataDir = fullfile('data','dir') ;
opts.imdbPath = '';
...
if(isfield(imdb.images,'data_mean'))
    opts.averageImage =imdb.images.data_mean;
end
...
res = [] ;
for epoch=start:opts.numEpochs
    ...
    for t=1:opts.batchSize:numel(train)
        ...
        [im, labels] = getBatch(input_net,opts,imdb, batch) ;
        if opts.prefetch
            nextBatch = train(t+opts.batchSize:min(t+2*opts.batchSize-1, numel(train))) ;
            getBatch(input_net,opts,imdb, nextBatch) ;
        end
        if opts.useGpu
            im = gpuArray(im) ;
        end
        ...
        res = vl_simplenn_autoencoder(net, im, one, res, ...
    ...
    if epoch == 1 || rem(epoch, opts.test_interval) == 0 || epoch == opts.numEpochs
        for t=1:opts.batchSize:numel(val)
            ...
            net.layers{end}.class = labels ;
            res = vl_simplenn_autoencoder(net, im, [], res, ...
                'disableDropout', true, ...
                'conserveMemory', opts.conserveMemory, ...
                'sync', opts.sync) ;
...
switch opts.errorType
    ...
    case 'euclideanloss'
        ...
        error = euclideanloss((predictions), labels);
